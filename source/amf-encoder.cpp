...
Plugin::AMD::Encoder::Encoder(Codec codec, std::shared_ptr<API::IAPI> videoAPI,
			      const API::Adapter &videoAdapter,
			      bool useOpenCLSubmission,
...
#pragma endregion Null Values
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Creating a AMF Context failed, error %ls (code %d).",
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Initializing %s API with Adapter '%s' failed, error %ls (code %d).",
			m_UniqueId, m_API->GetName().c_str(),
	...
	if (m_OpenCLSubmission || m_OpenCLConversion) {
		...
		if (res == AMF_OK) {
			...
			res = m_AMFContext->GetCompute(amf::AMF_MEMORY_OPENCL,
						       &m_AMFCompute);
			if (res != AMF_OK) {
				m_OpenCLSubmission = false;
				...
				QUICK_FORMAT_MESSAGE(
					errMsg,
					"<Id: %llu> Retrieving Compute object failed, error %ls (code %d)",
					m_UniqueId,
			...
			QUICK_FORMAT_MESSAGE(
				errMsg,
				"<Id: %llu> Initialising OpenCL failed, error %ls (code %d)",
				m_UniqueId,
				m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.data());
		}
	...
	// Create Converter
	res = m_AMFFactory->CreateComponent(m_AMFContext, AMFVideoConverter,
					    &m_AMFConverter);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Creating frame converter component failed, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Unable to set converter memory type, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Unable to set converter output format, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	res = m_AMFConverter->SetProperty(
		AMF_VIDEO_CONVERTER_COLOR_PROFILE,
		Utility::ColorSpaceToAMFConverter(m_ColorSpace));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Unable to set converter color profile, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	res = m_AMFConverter->SetProperty(
		AMF_VIDEO_CONVERTER_TRANSFER_CHARACTERISTIC,
		Utility::ColorSpaceToTransferCharacteristic(m_ColorSpace));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Unable to set converter transfer characteristic, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		PLOG_WARNING("%s", errMsg.c_str());
	}

	// Create Encoder
	res = m_AMFFactory->CreateComponent(
		m_AMFContext, Utility::CodecToAMF(codec), &m_AMFEncoder);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Unable to create %s encoder, error %ls (code %d)",
			m_UniqueId, Utility::CodecToString(codec),
			m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
...
void Plugin::AMD::Encoder::UpdateFrameRateValues()
{
	...
	m_FrameRateFraction =
		((double_t)m_FrameRate.second / (double_t)m_FrameRate.first);
...
void Plugin::AMD::Encoder::SetVBVBufferStrictness(double_t v)
{
	...
	strictBitrate = amf_clamp(
		static_cast<uint64_t>(
			round(targetBitrate * ((double_t)m_FrameRate.second /
					       (double_t)m_FrameRate.first))),
	...
	double_t aFade =
		(looseBitrate * (1.0 - aFadeVal)) + (targetBitrate * aFadeVal);
	double_t bFade =
		(aFade * (1.0 - bFadeVal)) + (strictBitrate * bFadeVal);
...
void Plugin::AMD::Encoder::Start()
{
	...
	res = m_AMFConverter->Init(Utility::ColorFormatToAMF(m_ColorFormat),
				   m_Resolution.first, m_Resolution.second);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Unable to initialize converter, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}

	res = m_AMFEncoder->Init(amf::AMF_SURFACE_NV12, m_Resolution.first,
				 m_Resolution.second);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Failed to initialize encoder, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
{
	AMF_RESULT res =
		m_AMFEncoder->ReInit(m_Resolution.first, m_Resolution.second);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Could not re-initialize encoder, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::Encoder::Stop()
{
	if (!m_Started)
		throw std::logic_error(
			"Can't stop an encoder that isn't running!");
...
bool Plugin::AMD::Encoder::Encode(struct encoder_frame *frame,
...
bool Plugin::AMD::Encoder::EncodeAllocate(OUT amf::AMFSurfacePtr &surface)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Unable to allocate Surface, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
	...
	uint64_t pf_timestamp =
		std::chrono::nanoseconds(clk_end.time_since_epoch()).count();
	uint64_t pf_time =
		std::chrono::nanoseconds(clk_end - clk_start).count();
...
bool Plugin::AMD::Encoder::EncodeStore(OUT amf::AMFSurfacePtr &surface,
				       IN struct encoder_frame *frame)
{
	...
	for (uint8_t i = 0; i < planeCount; i++) {
		...
		if (m_OpenCLSubmission) {
			...
			const amf_size l_size[] = {(amf_size)width,
						   (amf_size)height, 1};
			res = m_AMFCompute->CopyPlaneFromHost(
				frame->data[i], l_origin, l_size,
				frame->linesize[i], surface->GetPlaneAt(i),
				false);
			if (res != AMF_OK) {
				QUICK_FORMAT_MESSAGE(
					errMsg,
	...
	if (m_OpenCLSubmission) {
		...
		if (res != AMF_OK) {
			QUICK_FORMAT_MESSAGE(
				errMsg,
				"<Id: %llu> [Store] Failed to finish OpenCL queue, error %ls (code %d)",
				m_UniqueId,
				m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.data());
			return false;
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> [Store] Conversion of Surface failed, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		PLOG_WARNING("%s", errMsg.data());
		return false;
	}
	...
	if (m_Debug) {
		PLOG_DEBUG(
			"<Id: %llu> EncodeStore: PTS(%8lld) DTS(%8lld) TS(%16lld) Duration(%16lld) Type(%s)",
			m_UniqueId, frame->pts, frame->pts, surface->GetPts(),
...
bool Plugin::AMD::Encoder::EncodeConvert(IN amf::AMFSurfacePtr &surface,
					 OUT amf::AMFDataPtr &data)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> [Convert] Submit to converter failed, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		PLOG_WARNING("%s", errMsg.data());
		return false;
	}
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> [Convert] Querying output from converter failed, error %ls (code %d)",
			m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		PLOG_WARNING("%s", errMsg.data());
		return false;
	}
	if (m_OpenCLConversion) {
		...
		if (res != AMF_OK) {
			QUICK_FORMAT_MESSAGE(
				errMsg,
				"<Id: %llu> [Convert] Conversion of Surface failed, error %ls (code %d)",
				m_UniqueId,
				m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.data());
			return false;
...
bool Plugin::AMD::Encoder::EncodeMain(IN amf::AMFDataPtr &data,
				      OUT amf::AMFDataPtr &packet)
{
	...
	for (uint64_t attempt = 1; keepLooping; attempt++) {
		...
		if (!frameSubmitted) {
			if (m_MultiThreading) { // Asynchronous
				...
				uint64_t pf_ts = std::chrono::nanoseconds(
							 clk.time_since_epoch())
							 .count();
				data->SetProperty(AMF_TIMESTAMP_SUBMIT, pf_ts);

				AMF_RESULT res =
					m_AMFEncoder->SubmitInput(data);
				if (m_Debug) {
					QUICK_FORMAT_MESSAGE(
						errMsg,
						"<Id: %llu> [Main/Submit] SubmitInput returned %ls (code %d).",
						m_UniqueId,
						m_AMF->GetTrace()->GetResultText(
							res),
						res);
					PLOG_WARNING("%s", errMsg.c_str());
				}
				...
				if (res == AMF_OK) {
					...
					if (m_InitialFramesSent == false) {
						QUICK_FORMAT_MESSAGE(
							errMsg,
							"<Id: %llu> Queue Size is too large, starting to query for packets...",
							m_UniqueId);
						PLOG_ERROR("%s", errMsg.data());
					...
					QUICK_FORMAT_MESSAGE(
						errMsg,
						"<Id: %llu> [Main] Submitting Surface failed, error %ls (code %d)",
						m_UniqueId,
						m_AMF->GetTrace()->GetResultText(
							res),
						res);
		...
		if (m_InitialFramesSent && !packetRetrieved) {
			if (m_MultiThreading) {
				...
				AMF_RESULT res =
					m_AMFEncoder->QueryOutput(&packet);
				if (m_Debug) {
					QUICK_FORMAT_MESSAGE(
						errMsg,
						"<Id: %llu> [Main/Query] QueryOutput returned %ls (code %d).",
						m_UniqueId,
						m_AMF->GetTrace()->GetResultText(
							res),
						res);
				...
				if (res == AMF_OK) {
					...
					uint64_t
						...
						pf_submit, pf_main;
					packet->GetProperty(
						AMF_TIMESTAMP_SUBMIT,
						&pf_submit);
					packet->SetProperty(AMF_TIMESTAMP_QUERY,
							    pf_query);
					pf_main = (pf_query - pf_submit);
					packet->SetProperty(AMF_TIME_MAIN,
							    pf_main);
					...
					QUICK_FORMAT_MESSAGE(
						errMsg,
						"<Id: %llu> [Main] Retrieving Packet failed, error %ls (code %d)",
						m_UniqueId,
						m_AMF->GetTrace()->GetResultText(
							res),
						res);
	...
	if (!frameSubmitted) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> Input Queue is full, encoder is overloaded!",
			m_UniqueId);
		PLOG_WARNING("%s", errMsg.data());
	}
	if (!m_InitialPacketRetrieved) {
		QUICK_FORMAT_MESSAGE(errMsg,
				     "<Id: %llu> Waiting for initial frame...",
				     m_UniqueId);
	...
	if (m_InitialPacketRetrieved && !packetRetrieved) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %llu> No output Packet, encoder is overloaded!",
			m_UniqueId);
		PLOG_WARNING("%s", errMsg.data());
	}
...
bool Plugin::AMD::Encoder::EncodeLoad(IN amf::AMFDataPtr &data,
				      ...
				      OUT bool *received_packet)
{
	...
	/// Decode Timestamp
	packet->dts =
		(int64_t)round((double_t)data->GetPts() / m_TimestampStep) -
		m_TimestampOffset;
	/// Data
	...
	if (m_PacketDataBuffer.size() < packet->size) {
		size_t newBufferSize =
			(size_t)exp2(ceil(log2((double)packet->size)));
	...
	// Performance Tracking
	auto clk_end = std::chrono::high_resolution_clock::now();
	uint64_t pf_allocate_ts, pf_allocate_t, pf_store_ts, pf_store_t,
		pf_convert_ts, pf_convert_t, pf_submit_ts, pf_query_ts,
		pf_main_t, pf_load_ts, pf_load_t;
	...
	if (m_Debug) {
		...
		if (m_Codec == Codec::AVC || m_Codec == Codec::SVC) {
			...
			data->GetProperty(AMF_VIDEO_ENCODER_OUTPUT_DATA_TYPE,
					  &type);
		...
		if (m_Codec == Codec::HEVC) {
			...
			data->GetProperty(
				AMF_VIDEO_ENCODER_HEVC_OUTPUT_DATA_TYPE, &type);
			switch ((AMF_VIDEO_ENCODER_HEVC_OUTPUT_DATA_TYPE_ENUM)
					type) {
		...
		PLOG_DEBUG("<Id: %" PRIu64 "> EncodeLoad: PTS(%8" PRIu64
			   ") DTS(%8" PRIu64 ") TS(%16" PRIu64
			   ") Duration(%16" PRIu64 ") Size(%16" PRIuPTR
			   ") Type(%s)",
			   m_UniqueId, packet->pts, packet->dts, data->GetPts(),
			   data->GetDuration(), packet->size,
			   printableType.c_str());
		PLOG_DEBUG("<Id: %" PRIu64 ">    Timings: Allocate(%8" PRIu64
			   " ns) Store(%8" PRIu64 " ns) Convert(%8" PRIu64
			   " ns) Main(%8" PRIu64 " ns) Load(%8" PRIu64 " ns)",
			   m_UniqueId, pf_allocate_t, pf_store_t, pf_convert_t,
			   pf_main_t, pf_load_t);
	}
	if (m_InitialFrameLatency == 0) {
		m_InitialFrameLatency = pf_main_t;
		PLOG_INFO("<Id: %" PRIu64 "> Initial Frame Latency is %" PRIu64
			  " nanoseconds.",
			  m_UniqueId, m_InitialFrameLatency);
	}
...
int32_t Plugin::AMD::Encoder::AsyncSendLocalMain()
{
	...
	while (!own->shutdown) {
...

		{
			...
			uint64_t pf_ts =
				std::chrono::nanoseconds(clk.time_since_epoch())
					.count();
		...
		if (res == AMF_OK) {
			...
			QUICK_FORMAT_MESSAGE(
				errMsg,
				"<Id: %llu> Submitting Surface failed, error %ls (code %d)",
				m_UniqueId,
				m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_ERROR("%s", errMsg.data());
			return -1;
...
int32_t Plugin::AMD::Encoder::AsyncRetrieveLocalMain()
{
	...
	while (!own->shutdown) {
		...
		if (res == AMF_OK) {
			...
			// Performance Tracking
			{
				...
				uint64_t pf_query =
						 std::chrono::nanoseconds(
							 ...
							 .count(),
					 pf_submit, pf_main;
				packet->GetProperty(AMF_TIMESTAMP_SUBMIT,
						    &pf_submit);
				packet->SetProperty(AMF_TIMESTAMP_QUERY,
						    pf_query);
				pf_main = (pf_query - pf_submit);
			...
			QUICK_FORMAT_MESSAGE(
				errMsg,
				"<Id: %llu> Retrieving Packet failed, error %ls (code %d)",
				m_UniqueId,
				m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_ERROR("%s", errMsg.data());
			return -1;
