...
Plugin::AMD::Encoder::Encoder(Codec codec, std::shared_ptr<API::IAPI> videoAPI, const API::Adapter &videoAdapter, bool useOpenCLSubmission, bool useOpenCLConversion,
			      ColorFormat colorFormat, ColorSpace colorSpace, bool fullRangeColor, bool multiThreading, size_t queueSize)
{
...
#pragma endregion Null Values
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Creating a AMF Context failed, error %ls (code %d).", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Initializing %s API with Adapter '%s' failed, error %ls (code %d).", m_UniqueId, m_API->GetName().c_str(),
				     m_APIAdapter.Name.c_str(), m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	...
	if (m_OpenCLSubmission || m_OpenCLConversion) {
		...
		if (res == AMF_OK) {
			...
			if (res != AMF_OK) {
				...
				QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Retrieving Compute object failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res),
						     res);
			...
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Initialising OpenCL failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.data());
		}
	}
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Creating frame converter component failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	res = m_AMFConverter->SetProperty(AMF_VIDEO_CONVERTER_MEMORY_TYPE, amf::AMF_MEMORY_UNKNOWN);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to set converter memory type, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	res = m_AMFConverter->SetProperty(AMF_VIDEO_CONVERTER_OUTPUT_FORMAT, amf::AMF_SURFACE_NV12);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to set converter output format, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	res = m_AMFConverter->SetProperty(AMF_VIDEO_CONVERTER_COLOR_PROFILE, Utility::ColorSpaceToAMFConverter(m_ColorSpace));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to set converter color profile, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw AMFException(errMsg.c_str(), res);
	}
	res = m_AMFConverter->SetProperty(AMF_VIDEO_CONVERTER_TRANSFER_CHARACTERISTIC, Utility::ColorSpaceToTransferCharacteristic(m_ColorSpace));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to set converter transfer characteristic, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res),
				     res);
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to create %s encoder, error %ls (code %d)", m_UniqueId, Utility::CodecToString(codec),
...
void Plugin::AMD::Encoder::SetVBVBufferStrictness(double_t v)
{
	...
	strictBitrate = amf_clamp(static_cast<uint64_t>(round(targetBitrate * ((double_t)m_FrameRate.second / (double_t)m_FrameRate.first))), bitrateCaps.first, targetBitrate);
...
void Plugin::AMD::Encoder::Start()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to initialize converter, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}

	res = m_AMFEncoder->Init(amf::AMF_SURFACE_NV12, m_Resolution.first, m_Resolution.second);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Failed to initialize encoder, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
void Plugin::AMD::Encoder::Restart()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Could not re-initialize encoder, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
bool Plugin::AMD::Encoder::EncodeAllocate(OUT amf::AMFSurfacePtr &surface)
{
	...
	if (m_OpenCLSubmission) {
		...
		res = m_AMFContext->AllocSurface(amf::AMF_MEMORY_HOST, m_AMFSurfaceFormat, m_Resolution.first, m_Resolution.second, &surface);
	}
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Unable to allocate Surface, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
...
bool Plugin::AMD::Encoder::EncodeStore(OUT amf::AMFSurfacePtr &surface, IN struct encoder_frame *frame)
{
	...
	if (m_OpenCLSubmission) {
		...
		if (res != AMF_OK) {
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Store] Conversion of Surface to OpenCL failed, error %ls (code %d)", m_UniqueId,
					     m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.data());
			return false;
		}
	...
	for (uint8_t i = 0; i < planeCount; i++) {
		...
		if (m_OpenCLSubmission) {
			...
			if (res != AMF_OK) {
				QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Store] Unable to copy plane %d with OpenCL, error %ls (code %d)", m_UniqueId, i,
						     m_AMF->GetTrace()->GetResultText(res), res);
				PLOG_WARNING("%s", errMsg.data());
				return false;
			}
			...
			for (int32_t py = 0; py < height; py++) {
				...
				std::memcpy(static_cast<void *>(static_cast<uint8_t *>(plane_nat) + plane_off), static_cast<void *>(frame->data[i] + frame_off),
					    frame->linesize[i]);
			}
		}
	}
	...
	if (m_OpenCLSubmission) {
		...
		if (res != AMF_OK) {
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Store] Failed to finish OpenCL queue, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res),
					     res);
			PLOG_WARNING("%s", errMsg.data());
			return false;
		}
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Store] Conversion of Surface failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		PLOG_WARNING("%s", errMsg.data());
		return false;
	}
	...
	if (m_Debug) {
		PLOG_DEBUG("<Id: %llu> EncodeStore: PTS(%8lld) DTS(%8lld) TS(%16lld) Duration(%16lld) Type(%s)", m_UniqueId, frame->pts, frame->pts, surface->GetPts(),
			   surface->GetDuration(), printableType.c_str());
	}
...
bool Plugin::AMD::Encoder::EncodeConvert(IN amf::AMFSurfacePtr &surface, OUT amf::AMFDataPtr &data)
{
	...
	if (m_OpenCLConversion) {
		res = surface->Convert(amf::AMF_MEMORY_OPENCL);
		if (res != AMF_OK) {
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Convert] Conversion of Surface to OpenCL failed, error %ls (code %d)", m_UniqueId,
					     m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.data());
			return false;
		}
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Convert] Submit to converter failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		PLOG_WARNING("%s", errMsg.data());
		return false;
	}
	res = m_AMFConverter->QueryOutput(&data);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Convert] Querying output from converter failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res),
				     res);
		PLOG_WARNING("%s", errMsg.data());
		return false;
	}
	if (m_OpenCLConversion) {
		res = surface->Convert(m_AMFMemoryType);
		if (res != AMF_OK) {
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Convert] Conversion of Surface failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res),
					     res);
			PLOG_WARNING("%s", errMsg.data());
			return false;
		}
			...
			if (m_MultiThreading) { // Asynchronous
				...
				if (res == AMF_OK) {
					...
					if (m_InitialFramesSent == false) {
						QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Queue Size is too large, starting to query for packets...", m_UniqueId);
						PLOG_ERROR("%s", errMsg.data());
						m_InitialFramesSent = true;
					}
				} else {
					QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Main] Submitting Surface failed, error %ls (code %d)", m_UniqueId,
							     m_AMF->GetTrace()->GetResultText(res), res);
					PLOG_ERROR("%s", errMsg.data());
					return false;
				}
		...
		if (m_InitialFramesSent && !packetRetrieved) {
			if (m_MultiThreading) {
				...
				if (res == AMF_OK) {
					...
					QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Main] Retrieving Packet failed, error %ls (code %d)", m_UniqueId,
							     m_AMF->GetTrace()->GetResultText(res), res);
					PLOG_ERROR("%s", errMsg.data());
					return false;
				}
...
bool Plugin::AMD::Encoder::EncodeLoad(IN amf::AMFDataPtr &data, OUT struct encoder_packet *packet, OUT bool *received_packet)
{
	...
	// Performance Tracking
	auto clk_end = std::chrono::high_resolution_clock::now();
	uint64_t pf_allocate_ts, pf_allocate_t, pf_store_ts, pf_store_t, pf_convert_ts, pf_convert_t, pf_submit_ts, pf_query_ts, pf_main_t, pf_load_ts, pf_load_t;
	...
	if (m_Debug) {
		...
		PLOG_DEBUG("<Id: %" PRIu64 "> EncodeLoad: PTS(%8" PRIu64 ") DTS(%8" PRIu64 ") TS(%16" PRIu64 ") Duration(%16" PRIu64 ") Size(%16" PRIuPTR ") Type(%s)", m_UniqueId,
			   packet->pts, packet->dts, data->GetPts(), data->GetDuration(), packet->size, printableType.c_str());
	...
	if (m_InitialFrameLatency == 0) {
		...
		PLOG_INFO("<Id: %" PRIu64 "> Initial Frame Latency is %" PRIu64 " nanoseconds.", m_UniqueId, m_InitialFrameLatency);
	}
...
int32_t Plugin::AMD::Encoder::AsyncSendLocalMain()
{
	...
	while (!own->shutdown) {
		...
		if (m_Debug) {
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Main/Submit] SubmitInput returned %ls (code %d).", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.c_str());
		}
		...
		if (res == AMF_OK) {
			...
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Submitting Surface failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_ERROR("%s", errMsg.data());
			return -1;
		}
...
int32_t Plugin::AMD::Encoder::AsyncRetrieveLocalMain()
{
	...
	while (!own->shutdown) {
		...
		AMF_RESULT res = m_AMFEncoder->QueryOutput(&packet);
		if (m_Debug) {
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> [Main/Query] QueryOutput returned %ls (code %d).", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_WARNING("%s", errMsg.c_str());
		}
		...
		if (res == AMF_OK) {
			...
			QUICK_FORMAT_MESSAGE(errMsg, "<Id: %llu> Retrieving Packet failed, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
			PLOG_ERROR("%s", errMsg.data());
			return -1;
		}
