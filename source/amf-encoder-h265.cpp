...
Plugin::AMD::EncoderH265::EncoderH265(std::shared_ptr<API::IAPI> videoAPI, const API::Adapter &videoAdapter, bool useOpenCLSubmission, bool useOpenCLConversion,
				      ColorFormat colorFormat, ColorSpace colorSpace, bool fullRangeColor, bool useAsyncQueue, size_t asyncQueueSize)
	: Encoder(Codec::HEVC, videoAPI, videoAdapter, useOpenCLSubmission, useOpenCLConversion, colorFormat, colorSpace, fullRangeColor, useAsyncQueue, asyncQueueSize)
{
	this->SetUsage(Usage::Transcoding);

	AMF_RESULT res = res = m_AMFEncoder->SetProperty(L"NominalRange", m_FullColorRange);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "Failed to set encoder color range, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
	}
}
...
std::vector<Usage> Plugin::AMD::EncoderH265::CapsUsage()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Querying capabilities failed, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__,
...
void Plugin::AMD::EncoderH265::SetUsage(Usage v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::UsageToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
...
Plugin::AMD::Usage Plugin::AMD::EncoderH265::GetUsage()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to retrieve value, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, m_AMF->GetTrace()->GetResultText(res),
				     res);
		throw std::exception(errMsg.c_str());
	}
...
void Plugin::AMD::EncoderH265::SetQualityPreset(QualityPreset v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::QualityPresetToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetResolution(std::pair<uint32_t, uint32_t> v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %ldx%ld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v.first, v.second,
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
void Plugin::AMD::EncoderH265::SetAspectRatio(std::pair<uint32_t, uint32_t> v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %ld:%ld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v.first, v.second,
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
std::pair<uint32_t, uint32_t> Plugin::AMD::EncoderH265::GetFrameRate()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> Unable to retrieve value, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
void Plugin::AMD::EncoderH265::SetProfile(Profile v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::ProfileToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetProfileLevel(ProfileLevel v, std::pair<uint32_t, uint32_t> r, std::pair<uint32_t, uint32_t> h)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %lld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, (int64_t)v,
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetTier(H265::Tier v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::TierToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetMaximumReferenceFrames(uint64_t v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %lld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, m_AMF->GetTrace()->GetResultText(res),
				     res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetCodingType(CodingType v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::CodingTypeToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetMaximumLongTermReferenceFrames(uint32_t v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %ld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, m_AMF->GetTrace()->GetResultText(res),
				     res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetRateControlMethod(RateControlMethod v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::RateControlMethodToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetPrePassMode(PrePassMode v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__,
				     (v != PrePassMode::Disabled) ? "Enabled" : "Disabled", m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetVarianceBasedAdaptiveQuantizationEnabled(bool v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v ? "Enabled" : "Disabled",
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetVBVBufferInitialFullness(double v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %lf (%d), error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, (uint8_t)(v * 64),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetGOPType(H265::GOPType v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set mode to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::GOPTypeToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetMotionEstimationQuarterPixelEnabled(bool v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set mode to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v ? "Enabled" : "Disabled",
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetIFrameQPMinimum(uint8_t v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set to %d, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, m_AMF->GetTrace()->GetResultText(res),
				     res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH265::SetInputQueueSize(uint32_t v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, "<Id: %lld> <%s> Failed to set mode to %ld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v,
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
#pragma region Frame
	...
	PLOG_INFO(PREFIX "    Format: %s %s %s", m_UniqueId, Utility::ColorFormatToString(m_ColorFormat), Utility::ColorSpaceToString(m_ColorSpace),
...
#pragma region Static
	...
	PLOG_INFO(PREFIX "    Profile: %s %" PRIu16 ".%" PRIu16, m_UniqueId, Utility::ProfileToString(GetProfile()), profileLevel / 10, profileLevel % 10);
	...
	PLOG_INFO(PREFIX "    Max. Long-Term Reference Frames: %" PRIu16, m_UniqueId, (uint16_t)GetMaximumLongTermReferenceFrames());
...
#pragma region Flags
	...
	PLOG_INFO(PREFIX "        Behaviour: %s", m_UniqueId, GetFrameSkippingBehaviour() ? "Keep every Nth frame" : "Skip every Nth frame");
	PLOG_INFO(PREFIX "      Variance Based Adaptive Quantization: %s", m_UniqueId, IsVarianceBasedAdaptiveQuantizationEnabled() ? "Enabled" : "Disabled");
	PLOG_INFO(PREFIX "      Enforce Hypothetical Reference Decoder: %s", m_UniqueId, IsEnforceHRDEnabled() ? "Enabled" : "Disabled");
...
#pragma region Video Buffering Verifier
	...
	PLOG_INFO(PREFIX "      Initial Fullness: %" PRIu64 " %%", m_UniqueId, (uint64_t)round(GetInitialVBVBufferFullness() * 100.0));
...
#pragma region Picture Control
	...
	PLOG_INFO(PREFIX "    Motion Estimation: %s%s", m_UniqueId, IsMotionEstimationQuarterPixelEnabled() ? (IsMotionEstimationHalfPixelEnabled() ? "Quarter, " : "Quarter") : "",
