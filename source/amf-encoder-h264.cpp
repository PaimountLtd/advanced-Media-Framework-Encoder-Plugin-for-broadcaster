...
Plugin::AMD::EncoderH264::EncoderH264(
	...
	: Encoder(Codec::AVC, videoAPI, videoAdapter, useOpenCLSubmission,
		  ...
		  useAsyncQueue, asyncQueueSize)
{
	...
	if (m_AMF->GetRuntimeVersion() < AMF_MAKE_FULL_VERSION(1, 4, 0, 0)) {
		...
		AMF_RESULT res = res = m_AMFEncoder->SetProperty(
			L"NominalRange", m_FullColorRange);
		if (res != AMF_OK) {
		...
		AMF_RESULT res = res = m_AMFEncoder->SetProperty(
			AMF_VIDEO_ENCODER_FULL_RANGE_COLOR, m_FullColorRange);
		if (res != AMF_OK) {
			QUICK_FORMAT_MESSAGE(
				errMsg,
				PREFIX
...
std::vector<Usage> Plugin::AMD::EncoderH264::CapsUsage()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetPropertyInfo(AMF_VIDEO_ENCODER_USAGE, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
			"<%s> Querying capabilities failed, error %ls (code %d)",
	...
	for (const amf::AMFEnumDescriptionEntry *enm = var->pEnumDescription;
		...
		try {
			ret.push_back(Utility::UsageFromAMFH264(
				(AMF_VIDEO_ENCODER_USAGE_ENUM)enm->value));
		} catch (...) {
...
void Plugin::AMD::EncoderH264::SetUsage(Usage v)
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_USAGE,
						   Utility::UsageToAMFH264(v));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
			m_UniqueId, __FUNCTION_NAME__,
...
Plugin::AMD::Usage Plugin::AMD::EncoderH264::GetUsage()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
			"<%s> Failed to retrieve value, error %ls (code %d)",
			m_UniqueId, __FUNCTION_NAME__,
			m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
std::vector<QualityPreset> Plugin::AMD::EncoderH264::CapsQualityPreset()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_QUALITY_PRESET, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
	...
	for (const amf::AMFEnumDescriptionEntry *enm = var->pEnumDescription;
	     enm->name != nullptr; enm++) {
		try {
			ret.push_back(Utility::QualityPresetFromAMFH264(
...
Plugin::AMD::QualityPreset Plugin::AMD::EncoderH264::GetQualityPreset()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_QUALITY_PRESET, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
	...
	return Utility::QualityPresetFromAMFH264(
		(AMF_VIDEO_ENCODER_QUALITY_PRESET_ENUM)e);
}
...
std::vector<Profile> Plugin::AMD::EncoderH264::CapsProfile()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res =
		m_AMFEncoder->GetPropertyInfo(AMF_VIDEO_ENCODER_PROFILE, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
	...
	for (const amf::AMFEnumDescriptionEntry *enm = var->pEnumDescription;
	     enm->name != nullptr; enm++) {
		try {
			ret.push_back(Utility::ProfileFromAMFH264(
				(AMF_VIDEO_ENCODER_PROFILE_ENUM)enm->value));
		} catch (...) {
			// ignore unknown enum entries
		}
	}
...
void Plugin::AMD::EncoderH264::SetProfile(Profile v)
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_PROFILE, Utility::ProfileToAMFH264(v));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
Plugin::AMD::Profile Plugin::AMD::EncoderH264::GetProfile()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_PROFILE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::vector<ProfileLevel> Plugin::AMD::EncoderH264::CapsProfileLevel()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_PROFILE_LEVEL, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
					       ...
					       std::pair<uint32_t, uint32_t> h)
{
	...
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_PROFILE_LEVEL, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
			"<%s> Failed to set to %lld, error %ls (code %d)",
...
Plugin::AMD::ProfileLevel Plugin::AMD::EncoderH264::GetProfileLevel()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_PROFILE_LEVEL, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
Plugin::AMD::EncoderH264::CapsMaximumReferenceFrames()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_MAX_NUM_REFRAMES, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_MAX_NUM_REFRAMES, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
uint64_t Plugin::AMD::EncoderH264::GetMaximumReferenceFrames()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_MAX_NUM_REFRAMES, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
Plugin::AMD::EncoderH264::CapsResolution()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_FRAMESIZE, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
void Plugin::AMD::EncoderH264::SetResolution(std::pair<uint32_t, uint32_t> v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
			"<%s> Failed to set to %ldx%ld, error %ls (code %d)",
...
std::pair<uint32_t, uint32_t> Plugin::AMD::EncoderH264::GetResolution()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_FRAMESIZE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
void Plugin::AMD::EncoderH264::SetAspectRatio(std::pair<uint32_t, uint32_t> v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
			"<%s> Failed to set to %ld:%ld, error %ls (code %d)",
			m_UniqueId, __FUNCTION_NAME__, v.first, v.second,
			m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
std::pair<uint32_t, uint32_t> Plugin::AMD::EncoderH264::GetAspectRatio()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_ASPECT_RATIO, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint32_t, uint32_t> Plugin::AMD::EncoderH264::GetFrameRate()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_FRAMERATE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "Unable to retrieve value, error %ls (code %d)",
...
std::vector<CodingType> Plugin::AMD::EncoderH264::CapsCodingType()
{
	...
	for (const amf::AMFEnumDescriptionEntry *enm = var->pEnumDescription;
	     enm->name != nullptr; enm++) {
		try {
			ret.push_back(Utility::CodingTypeFromAMFH264(
...
Plugin::AMD::EncoderH264::CapsMaximumLongTermReferenceFrames()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_MAX_LTR_FRAMES, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_MAX_LTR_FRAMES, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %ld, error %ls (code %d)",
			m_UniqueId, __FUNCTION_NAME__, v,
			m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
uint32_t Plugin::AMD::EncoderH264::GetMaximumLongTermReferenceFrames()
{
	int64_t e;
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_MAX_LTR_FRAMES, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_HIGH_MOTION_QUALITY_BOOST_ENABLE, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
bool Plugin::AMD::EncoderH264::GetHighMotionQualityBoost()
{
	bool e;
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_HIGH_MOTION_QUALITY_BOOST_ENABLE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::vector<RateControlMethod> Plugin::AMD::EncoderH264::CapsRateControlMethod()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_RATE_CONTROL_METHOD, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
	...
	for (const amf::AMFEnumDescriptionEntry *enm = var->pEnumDescription;
	     enm->name != nullptr; enm++) {
		try {
			ret.push_back(Utility::RateControlMethodFromAMFH264(
...
void Plugin::AMD::EncoderH264::SetRateControlMethod(RateControlMethod v)
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		...
		Utility::RateControlMethodToAMFH264(v));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
Plugin::AMD::RateControlMethod Plugin::AMD::EncoderH264::GetRateControlMethod()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_RATE_CONTROL_METHOD, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
	...
	return Utility::RateControlMethodFromAMFH264(
		(AMF_VIDEO_ENCODER_RATE_CONTROL_METHOD_ENUM)e);
}
...
std::vector<PrePassMode> Plugin::AMD::EncoderH264::CapsPrePassMode()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_RATE_CONTROL_PREANALYSIS_ENABLE, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
	...
	for (const amf::AMFEnumDescriptionEntry *enm = var->pEnumDescription;
	     enm->name != nullptr; enm++) {
		try {
			ret.push_back(Utility::PrePassModeFromAMFH264(
...
void Plugin::AMD::EncoderH264::SetPrePassMode(PrePassMode v)
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		...
		Utility::PrePassModeToAMFH264(v));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
			m_UniqueId, __FUNCTION_NAME__,
			Utility::PrePassModeToString(v),
			m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
Plugin::AMD::PrePassMode Plugin::AMD::EncoderH264::GetPrePassMode()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_RATE_CONTROL_PREANALYSIS_ENABLE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "Unable to retrieve value, error %ls (code %d)",
	...
	return Utility::PrePassModeFromAMFH264(
		(AMF_VIDEO_ENCODER_PREENCODE_MODE_ENUM)e);
}

void Plugin::AMD::EncoderH264::SetVarianceBasedAdaptiveQuantizationEnabled(
	bool v)
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_ENABLE_VBAQ, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
bool Plugin::AMD::EncoderH264::IsVarianceBasedAdaptiveQuantizationEnabled()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_ENABLE_VBAQ, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_RATE_CONTROL_SKIP_FRAME_ENABLE, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
bool Plugin::AMD::EncoderH264::IsFrameSkippingEnabled()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_RATE_CONTROL_SKIP_FRAME_ENABLE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_ENFORCE_HRD, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
bool Plugin::AMD::EncoderH264::IsEnforceHRDEnabled()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_ENFORCE_HRD, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_FILLER_DATA_ENABLE, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
bool Plugin::AMD::EncoderH264::IsFillerDataEnabled()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_FILLER_DATA_ENABLE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint8_t, uint8_t> Plugin::AMD::EncoderH264::CapsQPMinimum()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res =
		m_AMFEncoder->GetPropertyInfo(AMF_VIDEO_ENCODER_MIN_QP, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
...
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_MIN_QP, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %d, error %ls (code %d)",
			m_UniqueId, __FUNCTION_NAME__, v,
			m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
uint8_t Plugin::AMD::EncoderH264::GetQPMinimum()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_MIN_QP, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint8_t, uint8_t> Plugin::AMD::EncoderH264::CapsQPMaximum()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res =
		m_AMFEncoder->GetPropertyInfo(AMF_VIDEO_ENCODER_MAX_QP, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %lld> <%s> Querying capabilities failed, error %ls (code %d)",
...
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_MAX_QP, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %d, error %ls (code %d)",
...
uint8_t Plugin::AMD::EncoderH264::GetQPMaximum()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_MAX_QP, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint64_t, uint64_t> Plugin::AMD::EncoderH264::CapsTargetBitrate()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_TARGET_BITRATE, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_TARGET_BITRATE, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
uint64_t Plugin::AMD::EncoderH264::GetTargetBitrate()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_TARGET_BITRATE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint64_t, uint64_t> Plugin::AMD::EncoderH264::CapsPeakBitrate()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_PEAK_BITRATE, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_PEAK_BITRATE, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
uint64_t Plugin::AMD::EncoderH264::GetPeakBitrate()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_PEAK_BITRATE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint8_t, uint8_t> Plugin::AMD::EncoderH264::CapsIFrameQP()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res =
		m_AMFEncoder->GetPropertyInfo(AMF_VIDEO_ENCODER_QP_I, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %lld> <%s> Querying capabilities failed, error %ls (code %d)",
...
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_QP_I, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %d, error %ls (code %d)",
...
std::pair<uint8_t, uint8_t> Plugin::AMD::EncoderH264::CapsPFrameQP()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res =
		m_AMFEncoder->GetPropertyInfo(AMF_VIDEO_ENCODER_QP_P, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %lld> <%s> Querying capabilities failed, error %ls (code %d)",
...
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_QP_P, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %d, error %ls (code %d)",
...
std::pair<uint8_t, uint8_t> Plugin::AMD::EncoderH264::CapsBFrameQP()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res =
		m_AMFEncoder->GetPropertyInfo(AMF_VIDEO_ENCODER_QP_B, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			"<Id: %lld> <%s> Querying capabilities failed, error %ls (code %d)",
...
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_QP_B, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %d, error %ls (code %d)",
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_MAX_AU_SIZE, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %ld, error %ls (code %d)",
...
uint32_t Plugin::AMD::EncoderH264::GetMaximumAccessUnitSize()
{
	int64_t e;
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_MAX_AU_SIZE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint64_t, uint64_t> Plugin::AMD::EncoderH264::CapsVBVBufferSize()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_VBV_BUFFER_SIZE, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_VBV_BUFFER_SIZE, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
uint64_t Plugin::AMD::EncoderH264::GetVBVBufferSize()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_VBV_BUFFER_SIZE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
void Plugin::AMD::EncoderH264::SetVBVBufferInitialFullness(double v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
			"<%s> Failed to set to %lf (%d), error %ls (code %d)",
...
float Plugin::AMD::EncoderH264::GetInitialVBVBufferFullness()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_INITIAL_VBV_BUFFER_FULLNESS, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res =
		m_AMFEncoder->SetProperty(AMF_VIDEO_ENCODER_IDR_PERIOD,
					  (int64_t)amf_clamp(v, 1, 1000000));
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %ld, error %ls (code %d)",
...
uint32_t Plugin::AMD::EncoderH264::GetIDRPeriod()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_IDR_PERIOD, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_HEADER_INSERTION_SPACING, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %ld, error %ls (code %d)",
...
uint32_t Plugin::AMD::EncoderH264::GetHeaderInsertionSpacing()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_HEADER_INSERTION_SPACING, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_DE_BLOCKING_FILTER, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
bool Plugin::AMD::EncoderH264::IsDeblockingFilterEnabled()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_DE_BLOCKING_FILTER, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_B_PIC_PATTERN, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %d, error %ls (code %d)",
...
uint8_t Plugin::AMD::EncoderH264::GetBFramePattern()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_B_PIC_PATTERN, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
int8_t Plugin::AMD::EncoderH264::GetBFrameDeltaQP()
{
	...
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(AMF_VIDEO_ENCODER_B_PIC_DELTA_QP, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_B_REFERENCE_ENABLE, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %s, error %ls (code %d)",
...
bool Plugin::AMD::EncoderH264::IsBFrameReferenceEnabled()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_B_REFERENCE_ENABLE, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_REF_B_PIC_DELTA_QP, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %d, error %ls (code %d)",
...
int8_t Plugin::AMD::EncoderH264::GetBFrameReferenceDeltaQP()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_REF_B_PIC_DELTA_QP, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_MOTION_HALF_PIXEL, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
			"<%s> Failed to set mode to %s, error %ls (code %d)",
			m_UniqueId, __FUNCTION_NAME__,
			v ? "Enabled" : "Disabled",
			m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
...
bool Plugin::AMD::EncoderH264::IsMotionEstimationQuarterPixelEnabled()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_MOTION_HALF_PIXEL, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_MOTION_QUARTERPIXEL, v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
bool Plugin::AMD::EncoderH264::IsMotionEstimationHalfPixelEnabled()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_MOTION_QUARTERPIXEL, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
std::pair<uint32_t, uint32_t>
Plugin::AMD::EncoderH264::CapsIntraRefreshNumMBsPerSlot()
{
	const amf::AMFPropertyInfo *var;
	AMF_RESULT res = m_AMFEncoder->GetPropertyInfo(
		AMF_VIDEO_ENCODER_INTRA_REFRESH_NUM_MBS_PER_SLOT, &var);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(
		AMF_VIDEO_ENCODER_INTRA_REFRESH_NUM_MBS_PER_SLOT, (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX "<%s> Failed to set to %ld, error %ls (code %d)",
...
uint32_t Plugin::AMD::EncoderH264::GetIntraRefreshNumMBsPerSlot()
{
	...
	AMF_RESULT res = m_AMFEncoder->GetProperty(
		AMF_VIDEO_ENCODER_INTRA_REFRESH_NUM_MBS_PER_SLOT, &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
{
	AMF_RESULT res = m_AMFEncoder->SetProperty(L"IntraRefreshNumOfStripes",
						   (int64_t)v);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
...
uint32_t Plugin::AMD::EncoderH264::GetIntraRefreshNumOfStripes()
{
	int64_t e;
	AMF_RESULT res =
		m_AMFEncoder->GetProperty(L"IntraRefreshNumOfStripes", &e);
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(
			errMsg,
			PREFIX
...
// Internal
void Plugin::AMD::EncoderH264::PacketPriorityAndKeyframe(
...
std::string Plugin::AMD::EncoderH264::HandleTypeOverride(amf::AMFSurfacePtr &d,
							 uint64_t index)
{
	AMF_VIDEO_ENCODER_PICTURE_TYPE_ENUM type =
	...
	if ((type != AMF_VIDEO_ENCODER_PICTURE_TYPE_NONE) &&
	    (m_PeriodIDR > 0) && ((index % m_PeriodIDR) == 0)) {
...
#pragma region QP
	...
	PLOG_INFO(PREFIX "      Range: %" PRIu8 " - %" PRIu8, m_UniqueId,
		  GetQPMinimum(), GetQPMaximum());
	PLOG_INFO(PREFIX "      I-Frame: %" PRIu8, m_UniqueId, GetIFrameQP());
	PLOG_INFO(PREFIX "      P-Frame: %" PRIu8, m_UniqueId, GetPFrameQP());
	try {
		PLOG_INFO(PREFIX "      B-Frame: %" PRIu8, m_UniqueId,
			  GetBFrameQP());
	} catch (...) {
...
#pragma region Picture Control
	...
	PLOG_INFO(PREFIX "      IDR: %" PRIu32 " Frames", m_UniqueId,
		  GetIDRPeriod());
	...
	PLOG_INFO(PREFIX "      B: %" PRIu32 " Frames", m_UniqueId,
		  GetBFramePeriod());
	PLOG_INFO(PREFIX "    Header Insertion Spacing: %" PRIu32, m_UniqueId,
		  GetHeaderInsertionSpacing());
	PLOG_INFO(PREFIX "    GOP Alignment: %s", m_UniqueId,
	...
	try {
		PLOG_INFO(PREFIX "      Pattern: %" PRIu8, m_UniqueId,
			  GetBFramePattern());
	} catch (...) {
		PLOG_INFO(PREFIX "      Pattern: N/A", m_UniqueId);
	}
	try {
		PLOG_INFO(PREFIX "      Delta QP: %" PRIi8, m_UniqueId,
			  GetBFrameDeltaQP());
	} catch (...) {
		PLOG_INFO(PREFIX "      Delta QP: N/A", m_UniqueId);
	}
	try {
		PLOG_INFO(PREFIX "      Reference: %s", m_UniqueId,
	...
	try {
		PLOG_INFO(PREFIX "      Reference Delta QP: %" PRIi8,
			  m_UniqueId, GetBFrameReferenceDeltaQP());
	} catch (...) {
...
#pragma region Intra - Refresh
	...
	PLOG_INFO(PREFIX "    Number of Macroblocks Per Slot: %" PRIu32,
		  m_UniqueId, GetIntraRefreshNumMBsPerSlot());
	PLOG_INFO(PREFIX "    Number of Stripes: %" PRIu32, m_UniqueId,
