...
Plugin::AMD::EncoderH264::EncoderH264(std::shared_ptr<API::IAPI> videoAPI, const API::Adapter &videoAdapter, bool useOpenCLSubmission, bool useOpenCLConversion,
				      ColorFormat colorFormat, ColorSpace colorSpace, bool fullRangeColor, bool useAsyncQueue, size_t asyncQueueSize)
	: Encoder(Codec::AVC, videoAPI, videoAdapter, useOpenCLSubmission, useOpenCLConversion, colorFormat, colorSpace, fullRangeColor, useAsyncQueue, asyncQueueSize)
{
...
std::vector<Usage> Plugin::AMD::EncoderH264::CapsUsage()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Querying capabilities failed, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, m_AMF->GetTrace()->GetResultText(res),
				     res);
...
void Plugin::AMD::EncoderH264::SetUsage(Usage v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::UsageToString(v),
...
Plugin::AMD::Usage Plugin::AMD::EncoderH264::GetUsage()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to retrieve value, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, m_AMF->GetTrace()->GetResultText(res),
				     res);
		throw std::exception(errMsg.c_str());
	}
...
void Plugin::AMD::EncoderH264::SetQualityPreset(QualityPreset v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::QualityPresetToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetProfile(Profile v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::ProfileToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetProfileLevel(ProfileLevel v, std::pair<uint32_t, uint32_t> r, std::pair<uint32_t, uint32_t> h)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %lld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, (int64_t)v,
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetMaximumReferenceFrames(uint64_t v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %lld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, m_AMF->GetTrace()->GetResultText(res),
				     res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetResolution(std::pair<uint32_t, uint32_t> v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %ldx%ld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v.first, v.second,
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
void Plugin::AMD::EncoderH264::SetAspectRatio(std::pair<uint32_t, uint32_t> v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %ld:%ld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v.first, v.second,
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
std::pair<uint32_t, uint32_t> Plugin::AMD::EncoderH264::GetFrameRate()
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "Unable to retrieve value, error %ls (code %d)", m_UniqueId, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
...
void Plugin::AMD::EncoderH264::SetCodingType(CodingType v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::CodingTypeToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetMaximumLongTermReferenceFrames(uint32_t v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %ld, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetHighMotionQualityBoost(bool v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v ? "Enabled" : "Disabled",
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetRateControlMethod(RateControlMethod v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::RateControlMethodToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetPrePassMode(PrePassMode v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, Utility::PrePassModeToString(v),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetQPMinimum(uint8_t v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %d, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetVBVBufferInitialFullness(double v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set to %lf (%d), error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v, (uint8_t)(v * 64),
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
...
void Plugin::AMD::EncoderH264::SetMotionEstimationQuarterPixelEnabled(bool v)
{
	...
	if (res != AMF_OK) {
		QUICK_FORMAT_MESSAGE(errMsg, PREFIX "<%s> Failed to set mode to %s, error %ls (code %d)", m_UniqueId, __FUNCTION_NAME__, v ? "Enabled" : "Disabled",
				     m_AMF->GetTrace()->GetResultText(res), res);
		throw std::exception(errMsg.c_str());
	}
}
